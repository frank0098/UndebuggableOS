...
static volatile uint8_t curr_ascii = 128;

/** @brief Copied from mp2. */
...
uint8_t set_font(uint16_t character) {
	...
	curr_ascii = (curr_ascii >= 254) ? 128 : curr_ascii + 2;

	for (i = 0; i < 16; i++) {
		font_data[curr_ascii][i] = gbk_font[get_gbk_offset(character) + i];
		font_data[curr_ascii + 1][i] = gbk_font[get_gbk_offset(character) + 16 + i];
	}
